...
if(inputPath["original"].length == 0) {
  Invalid(tr("Parameter") + " \"" + tr("Path to a folder or file on the device") + "\" " + tr("not specified."));
  return;
}
var outputPath = GetInputConstructorValue("outputPath", loader);
if(outputPath["original"].length == 0) {
  Invalid(tr("Parameter") + " \"" + tr("Path to a folder on the computer") + "\" " + tr("not specified."));
  return;
}
try {
  var code = loader.GetAdditionalData() + _.template($("#AndroidManager_pullFromDevice_code").html())({
